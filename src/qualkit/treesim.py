"""TreeSim scoring for GEOS decks. Vendored, not reimplemented.

This file is a byte-faithful copy of the scoring half of
``sci-sim-op/src/harness_evolve/simulators/geos.py`` (which is itself vendored
from ``siga/src/eval/judge_geos.py``), with the ``SimulatorSpec`` plumbing and
its imports removed so it has no dependency beyond the standard library.

**Do not edit this file.** The number it produces is the number the research
project reports; a local "improvement" to the metric makes your results
incomparable with ours, which is the one thing that would waste the whole
exercise. If you think the metric is wrong, say so in your write-up -- that is
a legitimate and welcome finding -- but do not change it here.

The entry point you want is :func:`evaluate_directories`; ``qualkit.scoring``
wraps it with the failures-as-zero convention.
"""
